...
<?php //printed die (hoffentlich) gehaltenen Stunden
    ...
    {//sucht Ferien mit Beginn oder Ende in diesem Monat
      if( ( substr($value['start'],5,2)=="$monthInt" && substr($value['start'],0,4)=="$year" ) || ( substr($value['end'],5,2)=="$monthInt" && substr($value['end'],0,4)=="$year") )
      {
        $start = strtotime($value['start']);
        $end = strtotime($value['end']);
      /* AUSGABE DER FERIEN
        $startRead = date('m/d/Y', $start);
        $endRead = date('m/d/Y', $end);
        echo "Diesen Monat($monthInt) sind Ferien von $startRead-$endRead"; */
      }
    }
