...
ini_set('display_startup_errors', 1);
  ...
  if(isset($_POST['vertreteneKurse']))
  {
    ...
    {//existiert der vertrenen Kurs?
      ...
      $kursname = $row[1];
      $sparte = $row[5];
      if($sparte=="") echo "$row[1] hat keine Sparte!!!";
    ...
    //console("start showing $data");
      foreach($data as $key => &$value)
      {
        ...
        if($gehaltenzahl < $trainerzahl && !$is_feiertag && !$is_ferientag) //wenn noch nicht alle eingetragen, Abrechnung
        {
          ...
          {
            //echo "Kurs neu eingegeben";
            $query = "INSERT INTO `gehalteneKurse` (`Kursnummer`, `Kurstitel`, `Datum`, `KursleiterTats1`, `KursleiterTats2`, `KursleiterTats3`, `KursleiterTats4`) VALUES ('$value[4]', '$value[5]', '$value[0]', '$user[0]',null,null,null);";
            $result = mysqli_query($connection,$query);
            echo mysqli_error($connection);
          }
